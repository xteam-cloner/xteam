...
async def join_call(chat_id: int, link: str, video: bool = False, resolution: int = 480):
    ...
    try:
        ...
        LOGS.error(f"Koneksi gagal atau terputus saat mencoba VC {chat_id}.")
        return 0

    except TelegramServerError:
        LOGS.error(f"Error server Telegram saat mencoba VC {chat_id}.")
        return 0
        
    except Exception as e:
        error_name = type(e).__name__
        
        if 'already' in str(e).lower() or 'already' in error_name.lower():
             LOGS.warning(f"Asisten sudah ada di VC {chat_id} (Dideteksi: {error_name}). Mencoba mengganti stream...")
             try:
        ...
        LOGS.error(f"Error fatal selama playback/join di {chat_id}: {error_name} - {e}")
        return 0
